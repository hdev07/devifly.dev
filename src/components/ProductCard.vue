  ...
  >
    ...
    <div class="relative h-44 overflow-hidden">
      ...
      <div
        ...
        style="background-image: radial-gradient(circle at 30% 70%, rgba(255,255,255,0.15) 0%, transparent 50%);"
      />
      <div class="absolute inset-0 flex items-center justify-center">
        <LucideIcon
          :name="product.icon"
          class-name="w-20 h-20 dark:text-white/10 text-white/30"
        />
      </div>
      <div
        class="absolute inset-x-0 bottom-0 h-20 bg-gradient-to-t dark:from-base-900/95 from-light-surface/95 to-transparent"
      />
    ...
    <div class="flex flex-col flex-1 p-5 sm:p-6">
      ...
      <ul class="space-y-2 mb-5">
        ...
        >
          <LucideIcon
            name="check"
            class-name="w-3.5 h-3.5 text-brand-400 shrink-0 mt-0.5"
          />
      ...
      <div class="mt-auto pt-4 border-t dark:border-white/8 border-light-border">
        <div class="flex items-end justify-between gap-3 mb-3">
          <div>
            <span
              class="block text-[10px] font-bold uppercase tracking-[0.18em] dark:text-text-secondary text-light-muted mb-1"
            >
              {{ fromLabel }}
            </span>
...
const gradientByKey = {
  ...
  custom: "bg-gradient-to-br from-slate-500/30 via-zinc-500/20 to-brand-500/20",
};
...
const accentByKey = {
  ...
  custom: "dark:text-slate-300 text-slate-600",
};

const props = defineProps({
  product: { type: Object, required: true },
  locale: { type: String, required: true },
  maxFeatures: { type: Number, default: null },
  compact: { type: Boolean, default: false },
  showHire: { type: Boolean, default: false },
  fromLabel: { type: String, default: "Desde" },
  viewMoreLabel: { type: String, default: "Ver más" },
  demoLabel: { type: String, default: "Demo" },
  hireLabel: { type: String, default: "Contratar" },
});

const bgGradient = computed(
  () => gradientByKey[props.product.key] ?? gradientByKey.landing
);
const accentText = computed(
  () => accentByKey[props.product.key] ?? accentByKey.landing
);

const visibleFeatures = computed(() => {
  const features =
    props.locale === "es" ? props.product.featuresEs : props.product.featuresEn;
  return props.maxFeatures ? features.slice(0, props.maxFeatures) : features;
});
</script>
