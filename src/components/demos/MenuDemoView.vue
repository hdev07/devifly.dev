<template>
  <div
    class="menu-demo"
    :class="{ 'search-open': showSearch, 'menu-demo--serif': theme.fontSerif }"
    :style="rootStyle"
  >
    <!-- Floating top bar -->
    <header
      ...
      :style="{
        ...
        borderBottom: `1px solid ${theme.colors.primaryFaint}`,
      }"
    >
      <div
      ...
      >
        ...
        <div class="flex items-center gap-2">
          ...
          >
            <span class="text-xs font-bold" :style="{ color: theme.colors.primary }">{{
              theme.logoInitial
            }}</span>
          </div>
          <span
            class="text-[10px] font-semibold tracking-widest uppercase px-2 py-1 rounded-full"
            :style="{
    ...
    <section
      ...
      :style="{ background: theme.colors.heroBg }"
    >
      <!-- Animated particles -->
      ...
      <div class="relative z-10 mt-24 mb-6 animate-fade-in-down">
        <div
          ...
          :style="{
            background: theme.colors.primaryGhost,
            border: `2px solid ${theme.colors.primaryFaint}`,
            boxShadow: `0 0 60px ${theme.colors.primaryGhost}`,
          }"
        >
          <span
      ...
      <h1
        class="relative z-10 font-bold text-3xl sm:text-4xl lg:text-5xl tracking-tight mb-2 animate-fade-in-up menu-title"
        :style="{
    ...
    <!-- Branch picker (Enterprise) -->
    <section
      v-if="theme.showBranchPicker"
      class="py-4 px-4"
      :style="{
    ...
    >
      <div class="max-w-lg lg:max-w-3xl mx-auto">
        <p
        ...
        <div class="flex gap-2 overflow-x-auto hide-scrollbar">
          <button
            v-for="branch in theme.branches"
            :key="branch.id"
            @click="activeBranch = branch.id"
            class="flex-shrink-0 px-4 py-2.5 rounded-xl text-xs font-semibold transition-all duration-300"
            :style="
              activeBranch === branch.id
                ? {
                    background: theme.colors.cta,
                    color: theme.colors.ctaText,
                    boxShadow: `0 4px 20px ${theme.colors.primaryGhost}`,
                  }
                : {
                    background: theme.colors.primaryGhost,
        ...
        <p
          v-if="selectedBranch"
          class="text-neutral-600 text-[11px] text-center mt-2"
        >
    ...
    <main class="pb-24" style="background: #0a0a0a">
      ...
      >
        ...
        >
          <div
            v-if="theme.showSubcategories && sub.name !== cat.name"
            class="max-w-lg lg:max-w-3xl mx-auto px-4 mb-3"
          >
            <div class="flex items-center gap-2">
              <div
                class="h-px flex-1"
          ...
          >
            <div
              ...
              "
              @click="theme.showItemModal && openDetail(item)"
            >
    ...
    <!-- Item detail modal -->
    <Transition v-if="theme.showItemModal" name="modal">
      <div
        v-if="selectedItem"
    ...
    >
      <div class="max-w-lg lg:max-w-3xl mx-auto px-4">
        ...
        <p class="text-neutral-600 text-xs mb-4">{{ theme.restaurantSubtitle }}</p>
...
const props = defineProps({
  tier: {
    type: String,
    required: true,
    validator: (v) => ["esencial", "pro", "premium"].includes(v),
  },
});

const theme = computed(() => menuThemes[props.tier]);
const waHire = computed(() => waLink(theme.value.waHire));
const waOrder = computed(() => waLink(theme.value.waOrder));
const promotions = computed(() => theme.value.promotions || []);

const rootStyle = computed(() => ({
  background: theme.value.colors.sectionBg,
  "--menu-primary": theme.value.colors.primary,
  "--menu-primary-light": theme.value.colors.primaryLight,
  "--menu-primary-muted": theme.value.colors.primaryMuted,
  "--menu-primary-faint": theme.value.colors.primaryFaint,
  "--menu-primary-ghost": theme.value.colors.primaryGhost,
}));
...
const activeBranch = ref(
  menuThemes.premium.branches ? menuThemes.premium.branches[0].id : null,
);
...
watch(
  () => props.tier,
  () => {
    activeCategory.value = menuCategories.value[0]?.id || "entradas";
  },
  { immediate: true },
);
